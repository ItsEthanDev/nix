...
6. **Derive vertical slices.** Break the design into dependency-ordered slices that each deliver and verify observable behavior. Introduce setup or shared foundational work only when it is a genuine prerequisite. Derive the task list from the accepted specification and plan, using `US-###` story identifiers and explicit `FR-###` and `SC-###` references from `project-documentation`.
