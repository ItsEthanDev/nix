...
1. **Load current state.** Read accepted behavior, technical design, execution state, applicable project rules, and relevant source and tests. Select the next unblocked slice and identify its `FR-###`, `SC-###`, and `US-###` references.
...
9. **Finish cleanly.** Run complete relevant test, build, lint, and format commands; inspect the diff; remove temporary instrumentation; reconcile artifact status; and record requirement-to-evidence results plus accepted deviations in the verification-result owner selected by `project-documentation`.
