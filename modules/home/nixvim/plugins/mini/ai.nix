...
in {
  programs.nixvim = {
    plugins.mini.modules.ai = {
      n_lines = 500;
      custom_textobjects = {
        o = mkRaw ''require("mini.ai").gen_spec.treesitter({ a = { "@block.outer", "@conditional.outer", "@loop.outer" }, i = { "@block.inner", "@conditional.inner", "@loop.inner" } })'';
        ...
        '';
        u = mkRaw ''require("mini.ai").gen_spec.function_call()'';
        U = mkRaw ''require("mini.ai").gen_spec.function_call({ name_pattern = "[%w_]" })'';
      };
    };
  };
}
