{ config, lib, ... }:
{
  ...
  options = {
    itsEthan.desktop.aerospace.enable = lib.mkEnableOption "Enables the aerospace module and customization";
  };

  config = lib.mkIf config.itsEthan.desktop.aerospace.enable {
    services.aerospace = {
      enable = true;
      settings = {
